...
function LazyArray(filnam::AbstractString, mtime::Real, begpos::Integer,
	...
	LazyArray{T,N}(String(filnam), Float64(mtime), Int64(begpos), hdu_type,
...
function LazyArray(filnam::AbstractString, mtime::Real, begpos::Integer,
	...
	LazyArray(filnam, mtime, begpos, Primary, format, fields, keywds)
end

function lazydata(hdu_type::Type, filnam::AbstractString, mtime::Real,
	begpos::Integer, format::DataFormat, fields, keywds::NamedTuple)

	if hdu_type in (Primary, Image, Conform)
		LazyArray(filnam, mtime, begpos, hdu_type, format, fields, keywds)
	elseif hdu_type in (Random, Table, Bintable)
		LazyStructuredData(String(filnam), Float64(mtime), Int64(begpos),
...
function read_bigendian(io::IO, type::Type)
	type <: Bool ? read(io, type) : ntoh(read(io, type))
end

function read_bigendian(io::IO, type::Type, n::Integer)
	ntoh.(reinterpret(type, read(io, sizeof(type)*n)))
end
...
function scale_value(value, field, scale::Bool)
	if scale && hasproperty(field, :zero) && hasproperty(field, :scale) &&
...
function lazy_eltype(type::Type, field, scale::Bool)
	...
	elseif type <: BitVector
		BitVector
	elseif scale && hasproperty(field, :zero) && hasproperty(field, :scale) &&
		!isnothing(getproperty(field, :zero)) && !ismissing(getproperty(field, :zero))
...
function contiguous_linear_span(shape::Tuple, ranges)
	...
	last_index - first_index + 1 == n ? (first_index:last_index) : nothing
end
...
function image_linear_value(io::IO, data::LazyArray, index::Integer)
	...
	scale_value(value, data.fields, get(data.keywds, :scale, true))
end
...
function DiskArrays.readblock!(data::LazyArray, out, ranges::OrdinalRange...)
	...
	try
		...
		if !isnothing(span)
			...
			out .= reshape(scaled_values(values, data.fields,
				get(data.keywds, :scale, true)), size(out))
...
function field_name(field)
	Symbol(rstrip(String(field.name)))
end

function field_groups(data::LazyStructuredData)
	names = field_name.(data.fields)
	[name => findall(==(name), names) for name in unique(names)]
end
...
function Base.propertynames(data::LazyStructuredData, private::Bool = false)
	...
	private ? (LAZY_STRUCTURED_INTERNALS..., names...) : names
end
...
function Base.length(data::LazyStructuredData)
	data.record ? structured_nrows(data) : length(keys(data))
end

function structured_nrows(data::LazyStructuredData)
	if data.hdu_type == Random
		data.format.group
	else
...
function LazyFieldArray(data::LazyStructuredData, fields::Vector)
	...
	LazyFieldArray{T,N}(data, fields)
end
...
function Base.getindex(data::LazyStructuredData, name::Union{Symbol, AbstractString})
	...
	LazyFieldArray(data, data.fields[groups[sym]])
end
...
function Base.getproperty(data::LazyStructuredData, name::Symbol)
	if name in LAZY_STRUCTURED_INTERNALS
		getfield(data, name)
...
function Base.read(data::LazyFieldArray)
	...
	try
		if parent.hdu_type == Random && length(data.fields) > 1
			cols = [read(io, field, parent.format, parent.begpos; parent.keywds...)
...
function read_lazy_field_row_value(data::LazyStructuredData, field, row::Integer)
	...
	if data.hdu_type == Random && field.leng > 1
